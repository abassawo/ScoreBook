...
class TestRepository : GameRepository() {
    ...
    companion object {
        ...
        val openGames = listOf(Game(name = "Game1", isClosed = true, players = players))
        val closedGames = listOf(Game(name = "Game2", isClosed = false, players = players, outcome = GameOutcome.WinnerAnnounced(
            player1)))
    }
